var promotionView = {
	...
	UpdateLike:function(){
		$(".likeScroll .swiper-wrapper").empty();
		 //show liked
        var l2 = store.likeList[store.currentCategoryId].length;
        var d = 0;
        ...
        if (l2 == 0) {
            ...
            var d = 0;
            while (d < l1 && d < l2) {
                if (store.promotions[store.likeList[store.currentCategoryId][d]] != null) {
                    $(".likeScroll .swiper-wrapper ").append(promotionView.PromotionItemSlide(store.likeList[store.currentCategoryId][d], -1));
                }
                d++;
            }
        }
        promotionView.currentLikeSlide = 0;
		scrolls["like"].reInit();
	},
    ...
    PromotionItemSlide: function (p, s) {
        ...
        var url = (store.partners[store.promotions[p].PartnerId] != null ? host + store.partners[store.promotions[p].PartnerId].Avatar : "img/default_partner.png");
        ...
        var like = 0;
        if (store.Contains(store.likeList[store.currentCategoryId], p)) {
            like = 1;
        }
        var add = (padd != null);
        if (percent > 0) {
