...
var detailControl = {
            ...
            client.GetDealCode(function (data) {
                ...
                if (data.status == 'ok') {
					if (data.data.remain == "-1") {
                        ...
                        $("#remain-text-1").removeClass("hidden");
                        $("#remain-text-2").removeClass("hidden");
                        $("#remain-text-1").html("(Còn lại " + data.data.remain + " mã)");
                        $("#remain-text-2").html("(Còn lại " + data.data.remain + " mã)");
                    }
    ...
    SendComment: function () {
        if (!userControl.loggedIn) {
            ...
            var s = $("#comment_text_area").html();
            if (s == '') {
                return;
            }
            $("#comment_text_area").html('');
            ...
            client.Comment(s, function (data) {
                ...
                store.promotions[data.data.promotionId].Comment = data.totalComment;
            },
         function (msg) { });
        }
    },
